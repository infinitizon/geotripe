    ...
    .controller('LogisticsList', ['$scope', '$localStorage', '$rootScope', '$state', '$stateParams', 'DataService','CommonServices'
        , function ($scope, $localStorage, $rootScope, $state, $stateParams, DataService, CommonServices) {
            ...
            vm.getData = function(pageno, filters) {
                 ...
                 */
                data.transactionMetaData.responseDataProperties = 'q.quote_id&q.rfq_no&p.name&CONCAT(LEFT(q.subject , 30),IF(LENGTH(q.subject)>30, "…", "")) subject&CONCAT(IFNULL(u.lastname,""),", ",IFNULL(u.middlename,"")," ",IFNULL(u.firstname,""))enteredBy&qs.name status&COUNT(qd.Quote_quote_Id) totalQuotes&DATEDIFF(q.duedate,NOW())remDays&q.entrydate&q.notes&qc.po_is_approved';
                data.transactionMetaData.pageno = pageno-1;
                data.transactionMetaData.itemsPerPage = vm.itemsPerPage;
                data.transactionMetaData.queryMetaData.queryClause.andExpression = [{
                    "propertyName": "q.po_no", "propertyValue": 'NULL', "propertyDataType": "VARCHAR", "operatorType": "IS"
                },{
                    "propertyName": "qc.po_is_approved", "propertyValue": 'NULL', "propertyDataType": "VARCHAR", "operatorType": "IS"
                }];
            ...
            vm.applyFilters = function(){
                ...
                if(angular.isDefined($scope.filters.rfqno)){
                    vm.filterOpts.andExpre.push({
                        "propertyName": "q.rfq_no", "propertyValue": $scope.filters.rfqno, "propertyDataType": "VARCHAR", "operatorType": "LIKE"
                    })
                }
